...
* manually add the stacks configuration
* load and parse the moves
* use the moves to remove one item from one stack and add it to the new stack
...
* extract the elements from the stack in solution 1
* create a temp stack where all removed items are stored
* reverse order of temp stack to preserve the order again
* add every item to the new stack